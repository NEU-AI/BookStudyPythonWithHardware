...
| 方法                              | 描述                              |
+-----------------------------------+-----------------------------------+
| `string.capitalize() <http://www. | 把字符串的第一个字符大写          |
| runoob.com/python/att-string-capi |                                   |
| talize.html>`__                   |                                   |
+-----------------------------------+-----------------------------------+
| `string.center(width) <http://www | 返回一个原字符串居中,并使用空格填充至长度为width的新字符串 |
| .runoob.com/python/att-string-cen |                                   |
| ter.html>`__                      |                                   |
+-----------------------------------+-----------------------------------+
| string.count(str,beg=0,end=len(st | 返回str在string里面出现的次数，如果beg或者end指定则 |
| ring))                            | 返回指定范围内str出现的次数       |
+-----------------------------------+-----------------------------------+
| `string.expandtabs(tabsize=8) <ht | 把字符串string中的tab符号转为空格，tab |
| tp://www.runoob.com/python/att-st |                                   |
| ring-expandtabs.html>`__          | 符号默认的空格数是 8。            |
+-----------------------------------+-----------------------------------+
| `string.find(str, beg=0,          | 检测str是否包含在string中，如果beg和end指定范围，则 |
| end=len(string)) <http://www.runo | 检查是否包含在指定范围内，如果是返回开始的索引值，否则返回-1 |
| ob.com/python/att-string-find.htm |                                   |
| l>`__                             |                                   |
+-----------------------------------+-----------------------------------+
| `string.format() <http://www.runo | 格式化字符串                      |
| ob.com/python/att-string-format.h |                                   |
| tml>`__                           |                                   |
+-----------------------------------+-----------------------------------+
...
| **序号**                          | **函数**                                |
+-----------------------------------+-----------------------------------------+
| 1                                 | `cmp(list1,list2) <http://www.runoob.co |
|                                   | m/python/att-list-cmp.html>`__\        |
|                                   | 比较两个列表的元素                       |
+-----------------------------------+-----------------------------------------+
